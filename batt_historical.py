...
solpos = pvlib.solarposition.get_solarposition(hourly_dataframe.index, latitude, longitude)
...
tilt_hourly = daily_tilt.reindex(hourly_dataframe.index.floor("D")).to_numpy()
...
daily_energy = hourly_dataframe["energy_Wh"].resample("D").sum().to_frame(name="historical_Wh")

charge_max, daily_budget_max = daily_soc(daily_energy, battery, SOC, reserve)
